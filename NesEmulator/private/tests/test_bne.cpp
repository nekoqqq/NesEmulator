...
static void test_bne_branch_taken() {
    ...
    // 测试2：向后跳转，Z=0，偏移 -5
    {
        ...
        std::cout << "[BNE] Branch backward test passed\n";
    }
    ...
    // 测试3：边界值 +127
    {
        ...
        assert(cpu.get_pc() == expected_pc);
        std::cout << "[BNE] Branch max forward (+127) passed\n";
    }
    ...
    // 测试4：边界值 -128
    {
        ...
        assert(cpu.get_pc() == expected_pc);
        std::cout << "[BNE] Branch max backward (-128) passed\n";
    }
}

static void test_bne_branch_not_taken() {
    // 测试5：Z=1 时不跳转
    {
        CPU cpu;
        cpu.reset_stats();
        cpu.set_flag(CPU::ZERO_FLAG, true);   // Z=1 不满足跳转
        cpu.set_pc(0x3000);
        ...
        std::cout << "[BNE] No branch when Z=1 passed\n";
    }
}

static void test_bne_page_cross() {
    // 跨页测试
    {
        CPU cpu;
        cpu.reset_stats();
        ...
        assert(cpu.get_pc() == expected_pc);
        std::cout << "[BNE] Page crossing branch passed\n";
    }
}

// ---------- 外部调用入口 ----------
void test_bne() {
    std::cout << "[BNE] Running tests...\n";
    test_bne_branch_taken();
    test_bne_branch_not_taken();
    test_bne_page_cross();
    std::cout << "[BNE] All tests passed.\n\n";
}