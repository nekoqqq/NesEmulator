...
static void test_bcc_branch_taken() {
    // 测试1：向前跳转，C=0，偏移 +10
    {
        CPU cpu;
        cpu.reset_stats();
        cpu.set_flag(CPU::CARRY_FLAG, false);   // C=0 满足跳转条件
        cpu.set_pc(0x2000);
        // 在 PC 指向的操作数位置写入偏移量
        cpu.mem_write(cpu.get_pc(), 10);        // 相对偏移 +10
        cpu.reset_stats();                     // 确保只清除了标志？这里我们单独设置 C
    ...
    // 测试2：向后跳转，C=0，偏移 -5（使用补码表示 251）
    {
        ...
        cpu.set_flag(CPU::CARRY_FLAG, false);
        cpu.set_pc(0x2000);
        cpu.mem_write(cpu.get_pc(), 0xFB);      // -5 的补码
        word old_pc = cpu.get_pc();
        OpCode::bcc(cpu, Relative);
...
static void test_bcc_branch_not_taken() {
    // 测试5：C=1 时不跳转
    {
        CPU cpu;
        cpu.reset_stats();
        cpu.set_flag(CPU::CARRY_FLAG, true);    // C=1 不满足跳转
        cpu.set_pc(0x3000);
